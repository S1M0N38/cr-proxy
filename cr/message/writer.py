...
class CrMessageWriter(BufferedWriter):

    def __init__(self, messageid, unknown):
        self._messageid = messageid
        self._unknown = unknown
        super(CrMessageWriter, self).__init__(BytesIO())
    ...
    def to_bytes(self):
        length = self.tell()
        return self._messageid.to_bytes(2, byteorder='big')
        + length.to_bytes(3, byteorder='big')
        + self._unknown.to_bytes(2, byteorder='big')
        + self.detach().getvalue()
