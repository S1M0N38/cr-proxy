...
from cr.message.reader import CrMessageReader
from cr.message.definitions import CrMessageDefinitions
import json
import math


class CrMessageDecoder:
    lengthTypes = ['BYTE', 'INT', 'RRSINT32']
    _bitfield = None

    def __init__(self):
        self._definitions = CrMessageDefinitions.read()
    ...
    def decode(self, messageid, unknown, payload):
        if messageid in self._definitions:
            ...
            raise KeyError('Message definition missing ({}).'
                           .format(messageid))
    ...
    def _decode_fields(self, reader, fields):
        ...
        for index, field in enumerate(fields):
            if 'name' not in field:
                field['name'] = 'unknown_{}'.format(
                    str(index).zfill(math.floor(math.log10(len(fields)))))

            # default in lengthType to 4 byte int.
            # Used to count the number of members in an array
            if 'lengthType' not in field:
                field['lengthType'] = 'INT'

            self._lengthTypeCheck(field['lengthType'])

            value = self._decode_field(reader, field['name'],
                                       field['type'], field['lengthType'])
    ...
    def _decode_field(self, reader, name, type, lengthType):
        ...
        if found >= 0:
            ...
            for i in range(int(count)):
                decoded.append(self._decode_field(
                    reader, '{}[{}]'.format(name, i), type, lengthType))
            return decoded
        elif type == 'BOOLEAN':
            ...
            decoded = self._decode_fields(
                reader, self._definitions['component'][type]['fields'])
            if 'extensions' in self._definitions['component'][type]:
                if not decoded['id'] in \
                   self._definitions['component'][type]['extensions']:
                    raise NotImplementedError(
                        '{}(id={}) has not yet been implemented.'
                        .format(type, decoded['id']))
                decoded['payload'] = self._decode_fields(
                    reader, self._definitions['component'][type]
                    ['extensions'][decoded['id']]['fields'])
            self.dump(decoded)
            return decoded
        else:
            raise NotImplementedError(
                '{} has not yet been implemented.'.format(type))

    def dump(self, decoded, hide_unknown=True):
        ...
        if 'fields' in decoded:
            print('{}: {}'.format(
                  decoded['name'],
                  json.dumps(self.stringify(decoded['fields'], hide_unknown),
                             indent=2)))
        else:
